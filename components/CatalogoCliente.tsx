...
export default function CatalogoCliente({ destinos, fechaInicial = "", viajerosIniciales = 1 }: { destinos: Destino[]; fechaInicial?: string; viajerosIniciales?: number }) {
  ...
  return <>
    ...
    <div className="mobile-snap-row sm:grid sm:grid-cols-2 sm:gap-6 lg:grid-cols-3">
      {destinosMostrados.map((d) => <article key={d.id} className="mobile-snap-card group flex min-h-[430px] flex-col overflow-hidden rounded-2xl border border-alm-beige-mid bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl dark:border-[#255369] dark:bg-[#133545] sm:w-auto sm:min-w-0">
    ...
    {seleccionado && <div role="dialog" aria-modal="true" aria-label={`Detalles de ${seleccionado.titulo}`} className="mobile-dialog-backdrop fixed inset-0 z-50 flex items-center justify-center bg-alm-dark/90 p-3 backdrop-blur-md" onMouseDown={(e) => { if (e.target === e.currentTarget) cerrar(); }}>
      <div className="mobile-bottom-sheet relative flex max-h-[94dvh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl dark:bg-alm-dark md:flex-row">
