...
export default function ContactCallbackWidget({ settings }: { settings: SiteSettings }) {
  ...
  async function submitRequest(event: FormEvent<HTMLFormElement>) {
    ...
    await supabase.from("solicitudes_contacto").insert(request);
    setSending(false);
    setSent(true);
  ...
  return <>
    <button type="button" onMouseEnter={() => setExpanded(true)} onMouseLeave={() => setExpanded(false)} onFocus={() => setExpanded(true)} onBlur={() => setExpanded(false)} onClick={() => { setOpen(true); setSent(false); }} className={`group fixed bottom-4 left-4 z-50 flex h-16 max-w-[calc(100vw-2rem)] items-center overflow-hidden rounded-2xl border-[3px] border-white bg-gradient-to-br from-alm-teal to-alm-mid text-white shadow-2xl transition-[width,transform] duration-500 dark:border-alm-dark sm:bottom-6 sm:left-6 sm:h-20 sm:rounded-3xl ${expanded ? "w-[min(23rem,calc(100vw-2rem))] -translate-y-1" : "w-16 sm:w-20"}`} aria-label={settings.contactPrompt}>
      <span className="flex h-full w-[3.8rem] shrink-0 items-center justify-center bg-white/95 p-1 sm:w-[4.8rem]"><Lottie animationData={mapAnimation} loop autoplay className="h-14 w-14 sm:h-[4.5rem] sm:w-[4.5rem]" aria-hidden /></span>
