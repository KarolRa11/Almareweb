      ...
      <div className="relative mx-auto max-w-6xl">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          {collection.packages.map((item, index) => (
            <article key={item.id} className={`group relative flex min-h-[390px] flex-col overflow-hidden rounded-3xl border border-t-[3px] border-white/70 bg-white shadow-[0_12px_35px_rgba(29,79,102,.10)] transition duration-300 hover:-translate-y-2 hover:shadow-[0_22px_45px_rgba(29,79,102,.18)] dark:border-white/10 dark:bg-[#123646] ${index === 2 ? "lg:-translate-y-3 lg:hover:-translate-y-5" : ""}`} style={{ borderTopColor: item.accent ?? "#4a9b8e", "--package-accent": item.accent ?? "#4a9b8e" } as CSSProperties}>
              <button type="button" onClick={() => setSelected(item)} aria-label={`Ver detalles de ${item.name}`} className="relative h-40 overflow-hidden bg-gradient-to-br from-alm-teal via-alm-mid to-alm-dark text-left" style={{ backgroundImage: `linear-gradient(135deg, ${item.accent ?? "#4a9b8e"}, #1d4f66)` }}>
